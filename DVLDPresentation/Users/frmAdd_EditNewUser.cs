...
{
    public partial class frmAdd_EditNewUser : Form
    {
        ...
        private  void _Save()
        {
            ...
            clsUsers.enMode prevMode = User.Mode;
            
            if (User.Save())
            {
                IsSave = true;
                if (prevMode == clsUsers.enMode.Update)
        ...
        void _UpdateUserModeFromEditOption()
        {
            ...
            if(User != null)
            {
                ...
                ctrlPersonCardWithFilter1.UpdateMode(clsUsers.Find(UserID).PersonID);
                _ChangeHeader("Update User");
                _FillDataFromObjectToForm();
            }
        }

        public frmAdd_EditNewUser(int UserID)
        {
            InitializeComponent();
            this.UserID = UserID;
            if(UserID != -1)
        ...
        private void gtxtUserName_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (clsUsers.IsUserExist(txt.Text) && User.UserName != txt.Text)
            {
